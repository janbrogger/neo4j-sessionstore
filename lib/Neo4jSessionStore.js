 ...
 getNeo4jSession() : Object {
  ...
  async get(sid: string, callback: Function): Promise<void> {
    try {
      ...
      neo4jsession.run(queryString, {sessionId: sessionId}).then((result => {
        this.closeNeo4jSession(neo4jsession);        
        
        if (result.records.length > 1) throw new Error("More than one session matched");
        const sessionId  = result.records.map((r) => {return r.get("n").properties['sessionId']});
        const expires  = fromSecondsEpoch(
          result.records.map((r) => {return r.get("n").properties['expires']})
        );
        const sess  = JSON.parse(
          result.records.map((r) => {return r.get("n").properties['sess']})
        );
