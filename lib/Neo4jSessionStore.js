...
export default class Neo4jSessionStore extends Store {  
  ...
  constructor(options?: Object = {}, callback?: Function = DEFAULT_CALLBACK) {
    ...
    process.on("exit", this.exitListen);
  }

  exitListen(code): void {
   ...
   */
  createTableIfDontExists(callback: Function): Promise<boolean> {
    ...
    try {
      ...
      await this.getSessionInternal(sid).then((result) => {
        if (result == null) {
          ...
          const queryString2 = `MATCH (n:${this.tableName} {sessionId: $sessionId }) SET n = $props RETURN n;`;
          neo4jsession
  ...
  async getSessionInternal(sid: string): Promise<Object> {
    try {
      ...
      const queryString = `MATCH (n:${this.tableName} {sessionId: $sessionId}) RETURN n;`;
      var record;

      return neo4jsession
        .run(queryString, { sessionId: sessionId })
        .then((result) => {
          this.closeNeo4jSession(neo4jsession);
  ...
  async destroy(sid: string, callback: Function): Promise<void> {
    try {
      const sessionId = this.getSessionId(sid);
      const neo4jsession = this.getNeo4jSession();
      const queryString = `MATCH (n:${this.tableName} {sessionId: $sessionId})  DETACH DELETE n;`;
  ...
  touch(sid: string, sess: Object, callback: Function): void {
    try {
      if (
        ...
        const neo4jsession = this.getNeo4jSession();
        const queryString = `MATCH (n:${this.tableName}) WHERE n.sessionId="${sessionId}" SET n += $props;`;
  ...
  close() {
    this.neo4jdriver.close();
    process.removeListener('exit', this.exitListen);
  }
}
